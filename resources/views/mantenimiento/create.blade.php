    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2>Crear Nueva Actividad de Mantenimiento</h2>
                ...
                <div class="card-body">
                    <form action="{{ route('mantenimiento.store') }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('frecuencia') is-invalid @enderror" 
                                        ...
                                        <option value="cuatrimestral" {{ old('frecuencia') == 'cuatrimestral' ? 'selected' : '' }}>Cuatrimestral</option>
                                        <option value="mensual" {{ old('frecuencia') == 'mensual' ? 'selected' : '' }}>Mensual</option>
                                        <option value="cuando_se_requiera" {{ old('frecuencia') == 'cuando_se_requiera' ? 'selected' : '' }}>Cuando se requiera</option>
                                    </select>
                                    @error('frecuencia')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('fecha_final') is-invalid @enderror" 
                                           id="fecha_final" name="fecha_final" value="{{ old('fecha_final') }}" required>
                                    @error('fecha_final')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <a href="{{ route('mantenimiento.index') }}" class="btn btn-secondary me-2">Cancelar</a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!fechaFinalInput.value) {
        fechaFinalInput.value = new Date().toISOString().split('T')[0];
    }
